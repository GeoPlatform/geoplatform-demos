...
sudo docker run -v $(pwd):/data --name GDAL --rm osgeo/gdal:alpine-small-latest ogr2ogr -overwrite -skipfailures -progress --config OGR_ORGANIZE_POLYGONS SKIP -lco SIGNIFICANT_FIGURES=6 -t_srs crs:84 -f GeoJSON /data/ak_$LAYER.geojson /data/AK_CadNSDI.gdb.zip CADNSDI_PLSSSECONDDIVISION
...
sudo docker run -v $(pwd):/data --name GDAL --rm osgeo/gdal:alpine-small-latest ogr2ogr -overwrite -skipfailures -progress --config OGR_ORGANIZE_POLYGONS SKIP -lco SIGNIFICANT_FIGURES=6 -t_srs crs:84 -f GeoJSON /data/co_$LAYER.geojson /data/BLM_CO_PLSS_20210823.gdb.zip BLM_CO_PLSS_SecondDivision
